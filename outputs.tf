locals {
  this_dnat_table_id           = local.this_nat_gateway_id != "" ? length(data.alicloud_nat_gateways.this.gateways) > 0 ? concat(data.alicloud_nat_gateways.this.gateways.*.forward_table_ids, [""])[0] : concat(alicloud_nat_gateway.this.*.forward_table_ids, [""])[0] : ""
  this_snat_table_id           = local.this_nat_gateway_id != "" ? length(data.alicloud_nat_gateways.this.gateways) > 0 ? concat(data.alicloud_nat_gateways.this.gateways.*.snat_table_ids, [""])[0] : concat(alicloud_nat_gateway.this.*.snat_table_ids, [""])[0] : ""
